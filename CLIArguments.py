...
def get_cli_arguents():
    ...
    # checking number of profiels to scrape is a valid integer - raise exception if not
    ...
    if args.profiles is not None:
        for profile_type in args.profiles:
            if profile_type not in GV.PROFILES.keys():
                exp = 'profile type {} not valid! check the help for this flag for avilable types'.format(profile_type)
                raise Exception(exp)
    ...
    if args.information is not None:
        for kind in args.information:
            if kind not in required_details:
                exp = 'information kind {} not valid! check the help for this flag for avilable types'.format(kind)
                raise Exception(exp)
